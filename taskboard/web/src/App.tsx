...
export function App() {
  ...
  // `?view=` deep-links the initial board view (e.g. embedded hosts that want
  // to open straight into the node workflow), bypassing the persisted view and
  // the SHOW_WORKFLOW_BOARD_ENTRY toolbar gate.
  const [boardView, setBoardView] = useState<BoardView>(() => {
    const requested = query.get("view");
    return requested === "dashboard" || requested === "issues" || requested === "list"
      || requested === "gantt" || requested === "workflow"
      ? requested
  ...
  useEffect(() => {
    // A `?view=` deep link owns the initial view; do not overwrite it with
    // the persisted one on first mount.
    if (selectedProjectId && !query.has("view")) setBoardView(readProjectBoardView(selectedProjectId));
